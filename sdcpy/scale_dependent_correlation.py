...
def generate_correlation_map(x: np.ndarray, y: np.ndarray, method: str = 'pearson') -> np.ndarray:
    ...
    if method.lower() not in ['spearman', 'pearson']:
        raise NotImplementedError(f'Method {method} not understood, must be one of "pearson", "spearman"')
...
def compute_sdc(ts1: np.ndarray, ts2: np.ndarray, fragment_size: int, n_permutations: int = 99,
                method: Union[str, Callable] = 'pearson', two_tailed: bool = True,
                permutations: bool = True, min_lag: int = -np.inf, max_lag: int = np.inf) -> pd.DataFrame:
    """
            ...
            if min_lag <= lag <= max_lag:
                ...
                if permutations:
                    ...
                    if method.lower() in ['pearson', 'spearman']:
                        ...
                        permuted_scores = [method_fun(np.random.permutation(fragment_1),
                                                      np.random.permutation(fragment_2))[0]
...
class SDCAnalysis:
    def __init__(self, ts1: np.ndarray, ts2: np.ndarray = None, fragment_size: int = 7, n_permutations: int = 99,
        ...
        if sdc_df is not None:
            ...
            self.sdc_df = (compute_sdc(self.ts1, self.ts2, fragment_size, n_permutations, method, two_tailed,
                                       permutations, min_lag, max_lag)
                           .assign(date_1=lambda dd: dd.start_1.map(self.ts1.reset_index().to_dict()['date_1']),
                                   date_2=lambda dd: dd.start_2.map(self.ts2.reset_index().to_dict()['date_2'])))
            self.sdc_df = self.sdc_df.loc[lambda dd: dd.start_1 != dd.start_2] if self.way == 'one-way' else self.sdc_df
        self.method = method

    def two_way_plot(self, alpha: float = .05, **kwargs):
    ...
    def to_excel(self, filename: str):
        with pd.ExcelWriter(filename) as writer:
            ...
            pd.concat([self.ts1.rename('ts1').reset_index().reset_index().rename(columns={'index': 'start_1'}),
                       self.ts2.rename('ts2').reset_index().reset_index().rename(columns={'index': 'start_2'})],
                      axis=1).to_excel(writer, sheet_name='time_series', index=False)

            pd.DataFrame({'fragment_size': self.fragment_size, 'n_permutations': self.n_permutations,
                          'method': self.method}, index=[1]).to_excel(writer, sheet_name='config', index=False)
    ...
    def from_excel(cls, filename: str):
        ...
        sdc_df = (pd.merge(pd.read_excel(filename, 'rs')
                           .melt('start_1', value_name='r', var_name='start_2'),
                           pd.read_excel(filename, 'p_values')
                           .melt('start_1', value_name='p_value', var_name='start_2'),
                           on=['start_1', 'start_2'])
                  .assign(stop_1=lambda dd: dd.start_1 + fragment_size,
    ...
    def dominant_lags_plot(self, alpha: float = .05, ylabel: str = '', **kwargs):
        fig, ax = plt.subplots(**kwargs)
        df = (self.sdc_df
              .loc[lambda dd: dd.p_value < alpha]
              .groupby('date_1')
              .apply(lambda dd: dd.loc[lambda ddd: ((ddd.r == ddd.r.max()) & (ddd.r > 0)) |
                                                   ((ddd.r == ddd.r.min()) & (ddd.r < 0))])
              .reset_index(drop=True)
              .groupby(['date_1']).apply(lambda dd: dd.loc[dd['lag'].abs() == dd['lag'].abs().min()])
              .assign(date_1=lambda dd: dd.date_1 + pd.to_timedelta(self.fragment_size // 2, unit='days'))
              .assign(lag=lambda dd: dd.lag.abs())
              )
        self.ts1.plot(ax=ax, color='black')
        ax2 = ax.twinx()
        sns.scatterplot(data=df, x='date_1', y='r', hue='lag', legend='full', alpha=.7, ax=ax2, palette='inferno_r')
        handles, labels = ax2.get_legend_handles_labels()
        ax2.legend(bbox_to_anchor=(1.3, 1.05), ncol=1, frameon=True, columnspacing=.2,
                   handles=[h for i, h in enumerate(handles[1:]) if i % 3 == 1],
                   labels=[l for i, l in enumerate(labels[1:]) if i % 3 == 1],
                   title='Lag')
